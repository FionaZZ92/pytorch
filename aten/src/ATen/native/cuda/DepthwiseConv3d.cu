...
__device__ __forceinline__ f16x2 pack_f16x2(c10::Half a, c10::Half b) {
  f16x2 v;
  reinterpret_cast<c10::Half*>(&v)[0] = a;
  reinterpret_cast<c10::Half*>(&v)[1] = b;
  return v;
}
    ...
    int dilationT_, int dilationH_, int dilationW_)
{
  ...
  scalar_t* s_input =
      reinterpret_cast<scalar_t*>(smem_base + WEIGHT_SIZE * sizeof(scalar_t));
  ...
  __syncthreads();
  scalar_t weight_reg[WEIGHT_SIZE];
  for (int w = 0; w < WEIGHT_SIZE; ++w) {
    weight_reg[w] = s_weight[w];
  }
...
#pragma unroll 2
  for (int out_linear = threadIdx.x; out_linear < num_outputs;
    ...
    scalar_t input_reg[WEIGHT_SIZE];
    {
      int wi_load = 0;
      for (int kf = 0; kf < kT; ++kf) {
        for (int kr = 0; kr < kH; ++kr) {
          for (int kc = 0; kc < kW; ++kc, ++wi_load) {
            ...
            input_reg[wi_load] = s_input[in_idx];
          }
        }
      }
    }
    float sum = 0.0f;
    //gfx950
#if __has_builtin(__builtin_amdgcn_fdot2_f32_bf16) && __has_builtin(__builtin_amdgcn_fdot2) 
...
#pragma unroll
    ...
    if constexpr (std::is_same_v<scalar_t, c10::Half>) {
      // 2 ▒~W f16 dot ▒~F~R f32 acc (AMDGCN dot2). Needs supported gfx + compile flags.
      using f16x2 = _Float16 __attribute__((ext_vector_type(2)));
#pragma unroll
      for (int w = 0; w + 1 < WEIGHT_SIZE; w += 2) {
        f16x2 wa = pack_f16x2(weight_reg[w + 0], weight_reg[w + 1]);
        f16x2 xi = pack_f16x2(input_reg[w + 0], input_reg[w + 1]);
    ...
    if constexpr (std::is_same_v<scalar_t, c10::Half>) {
      // 2 × f16 dot → f32 acc (AMDGCN dot2). Needs supported gfx + compile flags.
      using f16x2 = _Float16 __attribute__((ext_vector_type(2)));
#pragma unroll
      for (int w = 0; w + 1 < WEIGHT_SIZE; w += 2) {
        f16x2 wa = pack_f16x2(weight_reg[w + 0], weight_reg[w + 1]);
